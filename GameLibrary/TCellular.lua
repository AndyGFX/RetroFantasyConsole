...
---------------------------------------------------------------
function TCellular:PreviewMap(ox,oy,color0,color1)
	
	for xm=1,self._w do
		for ym=1,self._h do
			if (self:Map(xm,ym)==1) then
				RETRO:DrawPixel(RETRO:Vector2i(ox+xm,oy+ym),color1)
			else
				RETRO:DrawPixel(RETRO:Vector2i(ox+xm,oy+ym),color0)
			end
		end
	end
end
